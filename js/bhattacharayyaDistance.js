function BhattacharayyaDistance(vectors) {
	this.vectors = vectors;
}

BhattacharayyaDistance.prototype.calculate = function() {
	...
	for (var x = 0; x < this.vectors.length; x++) {
		for (var y = x + 1; y < this.vectors.length; y++) {
			...
			for (var d = 0; d < this.vectors[0].classification.length; d++)
				current_result += Math.sqrt(this.vectors[x].classification[d] * this.vectors[y].classification[d]);
